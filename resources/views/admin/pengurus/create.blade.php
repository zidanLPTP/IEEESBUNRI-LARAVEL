<x-layouts.admin>
    ...
    <div x-data="officerFormManager(@js($operationalDivisions), '{{ auth()->user()->accessRole ?? 'STAFF' }}')"
        class="relative w-full h-full pb-20">
        <template x-if="accessDenied">
            <div
                class="h-[80vh] flex flex-col items-center justify-center p-8 text-center animate-[fadeInUp_0.3s_ease-out]">
                <button @click="isSidebarOpen = true"
                    class="md:hidden absolute top-4 left-4 p-2 bg-[#151b2b] rounded-lg text-[#E7B95A] border border-white/10">
                ...
                <div
                    class="w-24 h-24 bg-red-500/10 text-red-500 rounded-full flex items-center justify-center mb-6 shadow-[0_0_30px_rgba(239,68,68,0.2)]">
                ...
                <p class="text-gray-400 max-w-md mb-8 leading-relaxed">
                    ...
                    <span class="text-white font-semibold">Only Head of Division and Core members can access this
                        page.</span>
                </p>
                <div class="flex gap-4">
                    <a href="{{ url('/admin/pengurus') }}"
                        class="px-6 py-3 rounded-xl border border-white/10 text-gray-300 hover:bg-white/5 font-bold text-sm transition-all">
                        Go Back
                    </a>
                </div>
            </div>
        ...
        <template x-if="!accessDenied">
            <form @submit="isSubmitting = true" action="{{ route('admin.pengurus.store') }}" method="POST"
                enctype="multipart/form-data">
                @csrf

                <div
                    class="sticky top-0 z-30 bg-[#0C101C]/80 backdrop-blur-md border-b border-white/5 px-4 md:px-8 py-5 flex justify-between items-center shadow-md gap-4 -mx-4 md:-mx-10 px-4 md:px-10 mb-8">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-[10px] text-gray-500 uppercase tracking-wide hidden md:block">Register Member
                                & Assign Roles</p>
                        </div>
                    </div>
                    ...
                    <button type="submit" :disabled="isSubmitting"
                        ...
                        <span x-show="isSubmitting" class="flex items-center gap-2" x-cloak>
                            <svg class="animate-spin h-4 w-4 text-[#0C101C]" xmlns="http://www.w3.org/2000/svg"
                ...
                @if ($errors->any())
                    <div
                        class="mb-6 mx-4 md:mx-0 p-4 bg-red-500/10 border border-red-500/50 rounded-xl text-red-500 text-sm">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    ...
                    <div class="lg:col-span-4 space-y-6">
                        <div class="bg-[#151b2b] border border-white/5 rounded-3xl p-6 md:p-8 text-center shadow-lg">
                            ...
                            <div class="mb-6 relative mx-auto w-40 h-40">
                                <div
                                    class="relative w-full h-full bg-[#0C101C] border-2 border-dashed border-white/10 rounded-full hover:border-[#E7B95A] transition-colors flex flex-col items-center justify-center cursor-pointer group overflow-hidden shadow-inner">
                                    ...
                                    <template x-if="!imagePreview">
                                        <div class="flex flex-col items-center">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32"
                                                ...
                                                class="text-gray-600 mb-2 group-hover:text-[#E7B95A] transition-colors">
                                                <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" />
                                                <polyline points="17 8 12 3 7 8" />
                                                <line x1="12" x2="12" y1="3" y2="15" />
                                            </svg>
                                            <span class="text-[10px] text-gray-500 uppercase font-bold">Upload
                                                Photo</span>
                                        </div>
                                    </template>
                            ...
                            <div class="space-y-4 text-left">
                                <div>
                                    <label class="text-xs font-bold text-gray-500 uppercase mb-2 block">IEEE Member ID
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="text" name="member_id" x-model="formData.memberId" required
                                            placeholder="10200..."
                                ...
                                <div class="p-4 rounded-xl border mt-4 transition-colors" :class="{
                    ...
                    <div class="lg:col-span-8 space-y-6">
                        <div class="bg-[#151b2b] border border-white/5 rounded-3xl p-6 md:p-8 shadow-lg">
                            ...
                            <div class="space-y-6">
                                <div>
                                    <label class="text-xs font-bold text-gray-500 uppercase mb-2 block">Full Name <span
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        <label
                                            class="text-xs font-bold text-gray-500 uppercase mb-2 block">Position</label>
                                        <div class="relative">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                                                viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                                stroke-linecap="round" stroke-linejoin="round"
                                            ...
                                            <select name="position" x-model="formData.position"
                                    ...
                                    <div>
                                        <label
                                            class="text-xs font-bold text-gray-500 uppercase mb-2 block">Department</label>

                                        <template x-if="showDivisionDropdown">
                                            <select name="division" x-model="formData.division"
                                        ...
                                        <template x-if="!showDivisionDropdown">
                                            <div
                                                class="w-full bg-white/5 border border-white/10 rounded-xl py-4 px-4 text-gray-400 italic cursor-not-allowed select-none flex items-center gap-2">
