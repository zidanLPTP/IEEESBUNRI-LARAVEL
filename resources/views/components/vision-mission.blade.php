...
<section class="relative py-32 bg-[#0C101C] text-white overflow-hidden">
    ...
    <div class="absolute top-0 left-1/2 -translate-x-1/2 w-[1px] h-full bg-[#3386B7]/10 z-0 overflow-hidden" aria-hidden="true">
        <div class="absolute left-0 w-full h-[150px] bg-gradient-to-b from-transparent via-white to-transparent opacity-50 animate-shooting-vision"></div>
    </div>
    ...
    <div class="container mx-auto px-6 relative z-10">
        ...
        <div class="text-center max-w-4xl mx-auto mb-20">
            ...
            <h2 class="text-3xl md:text-5xl font-medium leading-relaxed font-serif italic text-gray-200">
                "To strengthen IEEE SB UNRI as a <span class="text-white font-bold not-italic decoration-[#3386B7] underline decoration-4 underline-offset-4">professional, inclusive, and impactful</span> organization."
            </h2>
        </div>
            ...
            <div class="relative min-h-[450px] flex items-center justify-center">
                ...
                <div class="hidden md:flex justify-center items-center gap-6 w-full" style="perspective: 1000px;">
                    ...
                    @foreach($missions as $index => $item)
                        <div 
                            class="absolute w-1/3 min-w-[350px] h-[400px] transition-all duration-700 ease-out"
                            :class="{
                        ...
                        >
                            ...
                            <div class="relative w-full h-full p-10 rounded-3xl border transition-all duration-500 flex flex-col justify-center backdrop-blur-md"
                                ...
                                <h3 class="text-2xl font-bold mb-4 transition-colors" :class="active === {{ $index }} ? 'text-white' : 'text-gray-500'">
                                    {{ $item['title'] }}
                                </h3>
                                
                                <p class="text-sm leading-relaxed transition-colors" :class="active === {{ $index }} ? 'text-gray-300' : 'text-gray-600'">
                                    {{ $item['desc'] }}
                                </p>
                            </div>
                        </div>
                ...
                <div class="md:hidden relative w-full px-4 h-[350px]">
                    @foreach($missions as $index => $item)
                        ...
                        >
                            <div class="absolute inset-0 bg-gradient-to-r from-[#7AABC3] via-[#214664] to-[#E7B95A] animate-pulse"></div>
                            
                            <div class="relative bg-[#151b2b] p-8 rounded-3xl h-full flex flex-col justify-center">
                                <div class="w-14 h-14 bg-[#E7B95A] rounded-xl flex items-center justify-center text-[#0C101C] mb-6 shrink-0 shadow-lg">
            ...
            <div class="flex justify-center gap-3 mt-12 relative z-20">
                @foreach($missions as $index => $item)
                    <button
                        ...
                        :class="active === {{ $index }} ? 'w-12 bg-gradient-to-r from-[#3386B7] to-[#E7B95A]' : 'w-2 bg-gray-800 hover:bg-gray-600'"
                    ></button>
                @endforeach
            </div>
