...
<section class="relative py-32 bg-[#0C101C] text-white overflow-hidden">
    ...
    <div class="absolute top-0 left-1/2 -translate-x-1/2 w-[1px] h-full bg-[#3386B7]/10 z-0 overflow-hidden" aria-hidden="true">
         <div class="absolute top-0 left-0 w-full h-[150px] bg-gradient-to-b from-transparent via-white to-transparent opacity-40 animate-shooting-dept"></div>
    </div>

    <div class="container mx-auto px-6 relative z-10">
        
        <div class="text-center mb-32 relative" x-data="{ shown: false }" x-init="const observer = new IntersectionObserver(entries => { if (entries[0].isIntersecting) { shown = true; observer.disconnect(); } }, { rootMargin: '0px 0px -50px 0px' }); observer.observe($el);">
           <h2 :class="shown ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'" class="text-4xl md:text-6xl font-bold mb-4 transform transition-all duration-700 ease-out">
             Our <span class="text-transparent bg-clip-text bg-gradient-to-r from-[#7AABC3] to-[#3386B7]">Departments</span>
           </h2>
        </div>
        ...
        <div class="flex flex-col gap-32">
            @foreach($divisions as $index => $div)
                ...
                <div x-data="{ shown: false }" x-init="const observer = new IntersectionObserver(entries => { if (entries[0].isIntersecting) { shown = true; observer.disconnect(); } }, { rootMargin: '0px 0px -100px 0px' }); observer.observe($el);"
                    ...
                    <div class="w-full md:w-1/2 relative group perspective-1000">
                        ...
                        <div class="absolute -inset-[2px] rounded-3xl bg-gradient-to-r from-[#7AABC3] via-[#3386B7] via-[#214664] to-[#E7B95A] opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-[3px]" aria-hidden="true"></div>
                        ...
                        <div :class="shown ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-16'" class="relative aspect-[16/10] w-full rounded-3xl overflow-hidden border border-white/10 group-hover:border-transparent transition-all duration-700 ease-out shadow-2xl bg-[#151b2b]">
                            
                            <div class="absolute inset-0 bg-[#151b2b] animate-pulse z-0" aria-hidden="true"></div> 
                            
                            <img 
                                src="{{ asset('images/' . $div['image']) }}" 
                                alt="Ilustrasi Divisi {{ $div['name'] }} IEEE SB UNRI" 
                                loading="lazy"
                            ...
                            <div class="absolute bottom-3 right-4 text-[10px] text-white/50 bg-black/30 px-2 py-1 rounded-full backdrop-blur z-30">
                                Illustration by Storyset
                            </div>
                            
                            <div class="absolute inset-0 bg-gradient-to-t from-[#0C101C] via-transparent to-transparent opacity-80 z-20" aria-hidden="true"></div>
                            
                            <div class="absolute top-0 -left-[100%] w-full h-full bg-gradient-to-r from-transparent via-white/10 to-transparent group-hover:left-[100%] transition-all duration-1000 ease-in-out transform -skew-x-12 z-40" aria-hidden="true"></div>
                        </div>
                    </div>

                    <div class="w-full md:w-1/2 {{ $isEven ? 'text-left' : 'text-left md:text-right' }}">
                        
                        <div :class="shown ? 'opacity-100 scale-100' : 'opacity-0 scale-90'" class="inline-flex items-center justify-center p-4 rounded-2xl bg-[#3386B7]/5 border border-[#3386B7]/20 text-[#3386B7] mb-6 backdrop-blur-sm shadow-lg transform transition-all duration-500 delay-200 ease-out {{ !$isEven ? 'md:ml-auto' : '' }}">
                            {!! $div['icon'] !!}
                        </div>
                        
                        <h3 :class="shown ? 'opacity-100 translate-x-0' : 'opacity-0 {{ $isEven ? '-translate-x-8' : 'translate-x-8' }}'" class="text-3xl md:text-4xl font-bold text-white mb-6 hover:text-[#E7B95A] transition-all duration-500 delay-300">
                            {{ $div['name'] }}
                        </h3>
                        
                        <p :class="shown ? 'opacity-100 translate-x-0' : 'opacity-0 {{ $isEven ? '-translate-x-8' : 'translate-x-8' }}'" class="text-gray-400 text-lg leading-relaxed mb-8 font-light tracking-wide transition-all duration-500 delay-400">
                            {{ $div['desc'] }}
                        </p>

                        <div :class="shown ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'" class="inline-block group/btn transition-all duration-500 delay-500">
                            <a href="{{ url('/departments?tab=' . $div['id']) }}" class="relative block">
                                <div class="absolute -inset-[1.5px] rounded-full bg-gradient-to-r from-[#7AABC3] via-[#3386B7] via-[#214664] to-[#E7B95A] opacity-0 group-hover/btn:opacity-100 transition-opacity duration-300 blur-[2px]" aria-hidden="true"></div>
                                <button class="relative flex items-center gap-2 px-8 py-3 rounded-full border border-white/20 bg-[#0C101C] text-white group-hover/btn:border-transparent group-hover/btn:text-[#E7B95A] transform transition-all hover:scale-105 active:scale-95 font-semibold tracking-wide {{ !$isEven ? 'md:ml-auto' : '' }}">
                                    Explore Department 
