...
<section 
    id="events" 
    class="relative py-32 bg-[#0C101C] text-white overflow-hidden"
    x-data="{
        today: new Date(),
...
>
    ...
    <div class="absolute top-0 left-1/2 -translate-x-1/2 w-[1px] h-full bg-[#3386B7]/10 z-0 overflow-hidden" aria-hidden="true">
        <div class="absolute top-0 left-0 w-full h-[150px] bg-gradient-to-b from-transparent via-white to-transparent opacity-50 animate-[shootingLineDept_8s_linear_infinite]"></div>
    </div>
    ...
    <div class="container mx-auto px-6 relative z-10">
        ...
        >
            <div :class="shown ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-8'" class="transition-all duration-700 ease-out">
                <h2 class="text-4xl md:text-5xl font-bold mb-2">
                    Event <span class="text-transparent bg-clip-text bg-gradient-to-r from-[#7AABC3] to-[#3386B7]">Calendar</span>
                </h2>
                <p class="text-gray-400">Explore our agenda and activities.</p>
            </div>
            
            <div :class="shown ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'" class="flex items-center gap-3 transition-all duration-700 ease-out delay-200">
                <button 
                    x-show="currentMonth !== today.getMonth() || currentYear !== today.getFullYear()"
                    x-transition
                    @click="goToToday"
                ...
                <div class="flex items-center gap-4 bg-[#151b2b] p-2 rounded-full border border-white/10 shadow-lg">
                    ...
                    <span class="font-bold min-w-[140px] text-center" x-text="monthNames[currentMonth] + ' ' + currentYear"></span>
        ...
        <div class="grid lg:grid-cols-12 gap-12 items-stretch min-h-[500px]">
            ...
            <div class="lg:col-span-7 relative z-20">
                <div x-show="selectedEvent" x-transition class="h-full flex flex-col relative group w-full" x-cloak>
                    <div class="relative h-64 md:h-80 w-full rounded-3xl overflow-hidden mb-6 border border-white/10 group-hover:border-[#3386B7]/30 transition-colors shadow-lg">
                        <img 
                            :src="selectedEvent && selectedEvent.image ? selectedEvent.image : 'https://placehold.co/800x400/151b2b/7AABC3?text=IEEE+Event'" 
                            class="object-cover w-full h-full"
                        />
                        <div class="absolute inset-0 bg-gradient-to-t from-[#0C101C] via-[#0C101C]/50 to-transparent"></div>
                        <div class="absolute bottom-6 left-6 right-6">
                            <span 
                                ...
                                x-text="selectedEvent ? selectedEvent.realDivisionName : ''"
                            ></span>
                            <h3 class="text-3xl font-bold text-white group-hover:text-[#E7B95A] transition-colors" x-text="selectedEvent ? selectedEvent.title : ''"></h3>
                        </div>
                    </div>

                    <div class="bg-[#151b2b] p-6 rounded-2xl border border-white/5 flex-grow shadow-lg">
                        <p class="text-gray-300 mb-6 leading-relaxed whitespace-pre-wrap text-sm" x-text="selectedEvent ? selectedEvent.desc : ''"></p>
                        
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 bg-[#214664]/30 rounded-full flex items-center justify-center text-[#7AABC3] border border-[#7AABC3]/20">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/></svg>
                                </div>
                                <div>
                                    <p class="text-[10px] uppercase text-gray-500 font-bold">Location</p>
                                    <p class="text-sm font-bold" x-text="selectedEvent ? selectedEvent.speaker : ''"></p>
                                </div>
                            </div>
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 bg-[#214664]/30 rounded-full flex items-center justify-center text-[#7AABC3] border border-[#7AABC3]/20">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm font-bold" x-text="selectedEvent ? selectedEvent.date + ' ' + monthNames[currentMonth] + ' ' + currentYear : ''"></p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div x-show="!selectedEvent" class="h-full w-full flex flex-col items-center justify-center border border-dashed border-white/10 rounded-3xl bg-[#151b2b] p-10 text-center relative overflow-hidden">
                    <div class="relative mb-6">
                        <div class="absolute inset-0 bg-[#3386B7] blur-[40px] opacity-20 rounded-full"></div>
                        <svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="text-[#3386B7] relative z-10 opacity-80"><path d="M21 7.5V6a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h3.5"/><path d="M16 2v4"/><path d="M8 2v4"/><path d="M3 10h5"/><path d="M17.5 17.5 16 16.3V14"/><circle cx="16" cy="16" r="6"/></svg>
                    </div>
