<x-layouts.app>
    ...
    <main class="min-h-screen bg-[#0C101C] text-white font-sans selection:bg-[#E7B95A] selection:text-[#0C101C]">
        ...
        <section class="relative px-6 pt-32 mb-20 z-10">
            <div class="absolute top-0 right-0 w-[600px] h-[600px] bg-[#3386B7]/10 rounded-full blur-[150px] -z-10 pointer-events-none"
                ...
                <div class="flex flex-col lg:flex-row items-end justify-between gap-12">
                    ...
                    <div class="max-w-3xl">
                        <h1 class="text-5xl md:text-7xl font-extrabold leading-tight mb-6 animate-fade-in-up">
                            <br />Events
                            <span class="text-transparent bg-clip-text bg-gradient-to-r from-[#7AABC3] to-[#3386B7]"> &
                                Activities</span>
                        </h1>
                        <p class="text-gray-400 text-lg max-w-xl leading-relaxed animate-fade-in-up"
                            style="animation-delay: 100ms;">
                            Explore our schedule of upcoming seminars, workshops, and technical programs.
                        </p>
                    </div>

                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 w-full lg:w-auto min-w-[300px] animate-fade-in-up"
                        style="animation-delay: 200ms;">
                        <div
                            class="bg-[#151b2b] border border-white/5 p-6 rounded-2xl flex flex-col items-center justify-center text-center shadow-lg">
        ...
        <section class="py-20 px-6 relative z-20 animate-fade-in-up" style="animation-delay: 300ms;" x-data="{
                today: new Date(),
            ...
            <div class="container mx-auto">
                <div
                    class="bg-[#151b2b]/50 backdrop-blur-xl border border-white/10 rounded-[40px] p-8 md:p-12 shadow-2xl relative overflow-hidden">
                    <div class="grid lg:grid-cols-12 gap-12">
                        <div class="lg:col-span-5">
                            <div class="flex items-center justify-between mb-8">
                                <h2 class="text-2xl font-bold text-white"><span
                                        x-text="monthNames[currentMonth]"></span> <span x-text="currentYear"></span>
                                </h2>
                                <div class="flex gap-2">
                                    <button @click="prevMonth" class="p-2 hover:bg-white/10 rounded-full"><svg
                                            xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                            stroke-linecap="round" stroke-linejoin="round">
                                            <path d="m15 18-6-6 6-6" />
                                        </svg></button>
                                    <button @click="nextMonth" class="p-2 hover:bg-white/10 rounded-full"><svg
                                            xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                            stroke-linecap="round" stroke-linejoin="round">
                            ...
                            <div class="grid grid-cols-7 gap-4 text-center mb-4">
                                @foreach(['S', 'M', 'T', 'W', 'T', 'F', 'S'] as $d) <div
                                class="text-xs font-bold text-gray-500">{{ $d }}</div> @endforeach
                            </div>
                            <div class="grid grid-cols-7 gap-4 text-center">
                                ...
                                <template x-for="date in daysInMonth" :key="date">
                                    <button @click="selectEvent(date)"
                                        class="w-10 h-10 rounded-xl flex items-center justify-center text-sm transition-all"
                                        :class="getEvent(date) ? 'text-[#0C101C] font-bold shadow-lg scale-110' : 'text-gray-600 hover:bg-white/5'"
                                        :style="getEvent(date) ? `background-color: ${divisionColors[getEvent(date).divisionId] || '#E7B95A'}` : ''">
                        ...
                        <div class="lg:col-span-7 border-l-0 lg:border-l border-white/5 pl-0 lg:pl-12 pt-8 lg:pt-0">
                            <div x-show="selectedEvent" x-cloak x-transition>
                                <div class="bg-[#0C101C]/50 rounded-3xl p-6 border border-white/10 shadow-xl">
                                    <img :src="selectedEvent && selectedEvent.poster ? selectedEvent.poster : 'https://placehold.co/800x400/151b2b/7AABC3?text=IEEE+Event'"
                                    ...
                                    <template x-if="selectedEvent && selectedEvent.regLink">
                                        <a :href="selectedEvent.regLink" target="_blank"
                                            class="flex items-center justify-center w-full py-3.5 rounded-xl bg-[#E7B95A] text-[#0C101C] font-bold hover:brightness-110 transition-all">
                                            Register Now
                                        </a>
                                    </template>
                                </div>
                            ...
                            <div x-show="!selectedEvent"
                                class="h-full flex flex-col items-center justify-center text-center p-8 border-2 border-dashed border-white/10 rounded-3xl">
                                <h3 class="text-xl font-bold text-white mb-2">No Event Selected</h3>
                                <p class="text-gray-400 text-sm">Select a colored date on the calendar to see event
                                    details.</p>
                            </div>
                        </div>
                    </div>
        ...
        <section class="py-20 px-6 relative bg-[#05080f] border-t border-white/5 z-10">
            <div class="container mx-auto">
                ...
                @if($pastEvents->count() > 0)
                    <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                        @foreach($pastEvents as $idx => $event)
                            <article
                                class="bg-[#151b2b] border border-white/5 rounded-2xl p-6 hover:border-[#E7B95A]/30 transition-all animate-fade-in-up"
                                style="animation-delay: {{ $idx * 100 }}ms;">
                                <p class="text-xs text-[#E7B95A] mb-2 font-mono uppercase">
                ...
                @else
                    <div
                        class="w-full h-[300px] rounded-3xl border border-dashed border-white/10 flex flex-col items-center justify-center text-center">
                        <h3 class="text-2xl font-bold text-white mb-2">History in the Making</h3>
                        <p class="text-gray-400 max-w-md">Our journey log is currently empty. Exciting events are coming
                            soon!</p>
                    </div>
                @endif
            </div>
